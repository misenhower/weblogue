/*
 * Original Korg minilogue (OG) program init + serialization, bound to the OG
 * param table. Serialization is by stable param key so saved programs survive
 * id changes. Mirrors synths/xd/program.ts.
 */
...
export function deserializeProgram(json: string): Program | null {
  try {
    ...
    // v1 files (no synthId) predate the synth split and are xd programs, and
    // other synths' v2 programs are theirs — refuse both rather than silently
    // loading them as OG defaults.
    if (o.synthId !== SYNTH_ID) return null
