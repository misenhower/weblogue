/*
 * Raw-value -> physical-unit mappings, shared by the UI (display) and the DSP
 * (engine). Continuous panel knobs store hardware-style raw values 0..1023.
 * Piecewise tables reproduce the official minilogue xd MIDI implementation
 * (see docs/xd-spec.md).
 */
...
// ---------------------------------------------------------------------------
// Multi engine physical maps
// ---------------------------------------------------------------------------
export function noiseHighHz(shape01: number): number {
  return 10 * Math.pow(2100, clamp(shape01, 0, 1))
}
export function noiseLowHz(shape01: number): number {
  return 10 * Math.pow(2100, clamp(shape01, 0, 1))
}
export function noisePeakBwHz(shape01: number): number {
  return 110 * Math.pow(8, clamp(shape01, 0, 1))
}
export function noiseDecimHz(shape01: number): number {
...
export function gateTo01(gate: number): number {
  return gate >= GATE_TIE ? 1 : clamp(gate, 0, 72) / 72
}
...
// ---------------------------------------------------------------------------
// Micro tuning (cents offset per pitch class relative to equal temperament,
// rotated by scale key). Subset of the hardware list.
// ---------------------------------------------------------------------------
