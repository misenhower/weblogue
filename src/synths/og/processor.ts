...
/*
 * AudioWorklet processor for the original minilogue replica.
 *
 * Thin worklet shell around Engine (same protocol as the xd shell): routes
 * every ToEngine message in, posts FromEngine telemetry out (step / scope /
 * voices / dbg). No DOM imports — only shared/*, dsp/* and synths/og/*.
 * process() is exception-guarded so the processor can never die, handles any
 * block size, and always returns true.
 */
import { Engine, DBG_TAP_SIZE } from './engine'
import { SCOPE_SIZE } from '../../shared/messages'
import type { ToEngine } from '../../shared/messages'

/** Must match OG_DEF.processorName (synths/og/def.ts). */
...
class OgProcessor extends AudioWorkletProcessor {
  ...
  private onMessage(data: unknown): void {
    ...
    switch (m.t) {
      ...
      case 'bend':
        ...
        return
      case 'joyY': // the OG slider's assignable (non-bend) axis
      ...
      case 'sustain':
        ...
        return
      case 'pressure':
        this.engine.setPressure(m.v) // no-op: the OG has no aftertouch
        return
      case 'scope':
...
registerProcessor(OG_PROCESSOR_NAME, OgProcessor)
